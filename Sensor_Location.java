...
//		   s.nextLine();
		   ...
		   while(c!=2600){ //have it set to stop sending packets once 100 are sent
			   Thread.sleep(500); //pause for specified time
			
				//generating random values for sensors

				DecimalFormat two = new DecimalFormat("###.00");
				double latitude = client.getLoc1();
				double longitude = client.getLoc2();
...
//	   Sensor_Location rpacket = new Sensor_Location();
	   try{
	...
	int counter = 0;
      while(true){
		  if(counter == 0) {
		   ...
		   Sensor_Location client1=new Sensor_Location();
